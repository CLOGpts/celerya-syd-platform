        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center flex-shrink-0">
                    <div>
                        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Crea Risorsa QR (Scheda Tecnica)</h2>
                ...
                <footer className="p-4 border-t border-gray-200 dark:border-slate-700 flex justify-end items-center gap-3 flex-shrink-0 bg-white dark:bg-slate-800">
                    <button onClick={onClose} className="px-4 py-2 border border-gray-300 dark:border-slate-600 text-sm font-medium rounded-lg text-gray-700 dark:text-gray-200 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600">Annulla</button>
    ...
    return (
        <div className="fixed inset-0 bg-black/60 dark:bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center flex-shrink-0">
                    <div>
                        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Crea Risorsa QR (DDT)</h2>
                ...
                <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50 dark:bg-slate-900 relative">
                     <header className="mb-8 text-center border-b border-gray-300 dark:border-slate-600 pb-6">
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Documento di Trasporto</h2>
                        <p className="text-lg text-slate-700 dark:text-slate-300 mt-1">Mittente: {ddt.info.mittente}</p>
                     ...
                     <dl className="grid grid-cols-2 gap-x-4 gap-y-4 p-4 bg-white dark:bg-slate-800/50 rounded-lg border border-gray-200 dark:border-slate-700 mb-6">
                                ...
                                {ddt.prodotti.map((product, index) => {
                                    ...
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td className={`px-4 py-3 text-gray-600 dark:text-gray-400 ${isLottoMissing ? 'bg-red-50/75 dark:bg-red-900/20' : ''}`}>{isLottoMissing ? <MissingValue /> : product.lotto}</td>
                                            <td className={`px-4 py-3 text-gray-600 dark:text-gray-400 ${isScadenzaMissing ? 'bg-red-50/75 dark:bg-red-900/20' : ''}`}>{isScadenzaMissing ? <MissingValue /> : formattedDate}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
...
const SuppliersListPage: React.FC = () => {
    ...
    return (
        <div className="p-8">
            <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-6">Lista Fornitori</h1>
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                        {suppliers.length > 0 ? suppliers.map(supplier => (
                            <tr key={`${supplier.customerSlug}-${supplier.slug}`} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <button onClick={() => handleSupplierClick(supplier)} className="text-gray-800 dark:text-gray-200 font-medium hover:text-lime-600 dark:hover:text-lime-400 hover:underline text-left">
                                        {supplier.name}
                                    </button>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300 font-medium">{supplier.customerName}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">{supplier.docCount}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">{new Date(supplier.lastUpdate).toLocaleString('it-IT')}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    <button
                                        onClick={() => handleSupplierClick(supplier)}
                                        className="inline-flex items-center px-3 py-1.5 border border-gray-300 dark:border-slate-600 text-xs font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-lime-500 transition-colors"
                                    >
                                        Gestisci Documenti
                                    </button>
                                </td>
            ...
            {/* Drawer Panel */}
            <div className={`fixed top-0 right-0 h-full w-full max-w-xl bg-white dark:bg-slate-800 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                {selectedSupplier && (
                    <div className="flex flex-col h-full">
                        <header className="p-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-start bg-gray-50 dark:bg-slate-800">
                            <div>
                                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Documenti per {selectedSupplier.name}</h2>
                            ...
                            <button onClick={handleCloseDrawer} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-500 dark:text-gray-400 hover:text-gray-800 transition-colors"><XIcon className="w-6 h-6" /></button>
                        </header>
                            ...
                            {activeTab === 'pdfs' && (
                                <ul className="space-y-3">
                                    {sortedPdfs.length > 0 ? sortedPdfs.map(pdf => {
                                        ...
                                        return (
                                            <li key={pdf.id} className="p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm gap-3">
                                                <div className="flex-grow">
                                                    <p className="font-semibold text-gray-800 dark:text-gray-200">{pdf.identificazione.denominazioneScheda}</p>
                                                    ...
                                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">ID Risorsa: <span className="font-mono bg-gray-100 dark:bg-slate-700 px-1 py-0.5 rounded">{pdf.id}</span></p>
                                                </div>
                                                <div className="flex items-center gap-2 self-end sm:self-center flex-shrink-0">
                                                    {pdf.qrCodeUrl ? (
                                                        <div className="flex items-center gap-2 text-green-600 dark:text-green-400 text-xs font-bold bg-green-50 dark:bg-green-500/10 py-1 px-2 rounded-full">
                                                    ...
                                                    <button onClick={() => handleDownloadPdfClick(pdf)} disabled={isGenerating && pdfToGenerate?.product.id === pdf.id} className="flex items-center justify-center p-2 border border-transparent text-sm font-medium rounded-md text-slate-600 bg-slate-100 dark:bg-slate-200/20 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-200/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-colors disabled:bg-slate-400 disabled:cursor-wait" title="Scarica PDF">
                             ...
                             {activeTab === 'ddts' && (
                                <ul className="space-y-3">
                                    {sortedDdts.length > 0 ? sortedDdts.map(ddt => {
                                        ...
                                        return (
                                            <li key={ddt.id} className="p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm gap-3">
                                                <div className="flex-grow">
                                                    <p className="font-semibold text-gray-800 dark:text-gray-200">DDT: {ddt.info.numeroDDT || 'N/D'}</p>
                                                    <p className="text-xs text-gray-500 dark:text-gray-400">Data: {ddtDate} &middot; Dest: {ddt.info.destinatario}</p>
                                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">ID Risorsa: <span className="font-mono bg-gray-100 dark:bg-slate-700 px-1 py-0.5 rounded">{ddt.id}</span></p>
                                                </div>
                                                <div className="flex items-center gap-2 self-end sm:self-center flex-shrink-0">
                                                    {ddt.qrCodeUrl ? (
                                                        <div className="flex items-center gap-2 text-green-600 dark:text-green-400 text-xs font-bold bg-green-50 dark:bg-green-500/10 py-1 px-2 rounded-full">
                            ...
                            {activeTab === 'catalogs' && (
                                <ul className="space-y-3">
                                    {sortedCatalogs.length > 0 ? sortedCatalogs.map(catalog => (
                                        <li key={catalog.id} className="p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm gap-3">
                                            <div className="flex-grow flex items-center gap-3">
                                                <CatalogIcon className="w-6 h-6 text-slate-500 dark:text-slate-400 flex-shrink-0" />
                                                <div>
                                                    <p className="font-semibold text-gray-800 dark:text-gray-200">{catalog.nomeCatalogo}</p>
                            ...
                            {activeTab === 'priceLists' && (
                                <ul className="space-y-3">
                                    {sortedPriceLists.length > 0 ? sortedPriceLists.map(priceList => (
                                        <li key={priceList.id} className="p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm gap-3">
                                            <div className="flex-grow flex items-center gap-3">
                                                <PriceListIcon className="w-6 h-6 text-slate-500 dark:text-slate-400 flex-shrink-0" />
                                                <div>
                                                    <p className="font-semibold text-gray-800 dark:text-gray-200">{priceList.nomeListino}</p>
