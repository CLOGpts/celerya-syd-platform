            ...
            <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden">
                ...
                <main className="p-6 space-y-8">
                    {activeSections.map(section => (
                        <div key={section.key}>
                            <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100 border-b-2 border-lime-500 pb-2 mb-4">{section.title}</h2>
                            <dl>
                                {section.data.map(item => {
                                    const isImportant = importantLabels.has(item.label);
                                    return (
                                        <div key={item.key} className="py-3 grid grid-cols-1 md:grid-cols-3 gap-1 md:gap-4 border-b border-gray-200 dark:border-slate-700 last:border-b-0">
                                            <dt className={`text-sm ${isImportant ? 'font-bold text-slate-800 dark:text-slate-200' : 'font-semibold text-gray-600 dark:text-gray-400'}`}>{item.label}</dt>
                                            <dd className="text-sm text-gray-800 dark:text-gray-300 mt-1 md:mt-0 md:col-span-2 whitespace-pre-wrap">{renderValue(item.value, item.key, item.label)}</dd>
                                        </div>
                                    );
                                })}
                            </dl>
                ...
                <footer className="p-6 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-200 dark:border-slate-700 text-center">
                    <button
