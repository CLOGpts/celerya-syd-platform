        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm h-full flex flex-col col-span-1 md:col-span-2">
            <div className="flex justify-between items-center mb-6 gap-4">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">Risultati Analisi Listino</h1>
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-100 dark:bg-slate-700 sticky top-0">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-semibold text-gray-600 dark:text-gray-300">Prezzo Netto</th>
                            <th className="px-4 py-3 font-semibold text-gray-600 dark:text-gray-300">Valuta</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                        {data.items.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                <td className="px-4 py-3 text-gray-600 dark:text-gray-400 font-mono">{item.codiceArticolo}</td>
                                <td className="px-4 py-3 text-gray-800 dark:text-gray-200 font-medium">{item.descrizione}</td>
                                <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{item.prezzoNetto?.toFixed(2)}</td>
                                <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{item.valuta}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
