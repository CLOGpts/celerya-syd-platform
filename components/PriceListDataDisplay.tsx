...
import type { AnalyzedPriceList } from '../types';
import * as XLSX from 'xlsx';
import { ExcelIcon } from './icons/ExcelIcon';
import { ArrowLeftIcon } from './icons/ArrowLeftIcon';
import DocumentSplitView from './DocumentSplitView';

interface PriceListDataDisplayProps {
  data: AnalyzedPriceList;
...
const PriceListDataDisplay: React.FC<PriceListDataDisplayProps> = ({ data, onAnalyzeAnother, customerName, onBack, originalFileUrl }) => {
    const handleDownloadExcel = () => {
        ...
        const filename = `Listino - ${data.nomeListino.replace(/[\/\\]/g, '_')} - ${new Date().toISOString().split('T')[0]}.xlsx`;
            ...
            <div className="bg-gray-900 dark:bg-slate-800 p-6 h-full flex flex-col">
                <div className="flex justify-between items-center mb-6 gap-4">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-200 dark:text-gray-100">Risultati Analisi Listino</h1>
                ...
                <dl className="grid grid-cols-2 gap-x-4 gap-y-4 p-4 bg-black dark:bg-slate-700/50 rounded-lg border border-gray-800 dark:border-slate-700 mb-6">
                    ...
                    <InfoItem label="Nome Listino" value={data.nomeListino} />
                    <div className="col-span-2">
                        <InfoItem label="Data di Validità" value={formatDate(data.dataValidita) ?? 'N/D'} />
                    </div>
                </dl>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-900 dark:bg-slate-700 sticky top-0">
                            <tr>
                                ...
                                <th className="px-4 py-3 font-semibold text-gray-400 dark:text-gray-300">Prezzo Netto</th>
                                <th className="px-4 py-3 font-semibold text-gray-400 dark:text-gray-300">Valuta</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                            {data.items.map((item, index) => (
                                <tr key={index} className="hover:bg-black dark:hover:bg-slate-700/50">
                                    <td className="px-4 py-3 text-gray-400 dark:text-gray-400 font-mono">{item.codiceArticolo}</td>
                                    <td className="px-4 py-3 text-gray-200 dark:text-gray-200 font-medium">{item.descrizione}</td>
                                    <td className="px-4 py-3 text-gray-400 dark:text-gray-400">{item.prezzoNetto?.toFixed(2)}</td>
                                    <td className="px-4 py-3 text-gray-400 dark:text-gray-400">{item.valuta}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
export default PriceListDataDisplay;
