  ...
  return (
    <div className="min-h-screen bg-gray-900 dark:bg-slate-900 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-slate-100 dark:text-white mb-2">
        ...
        <div className="bg-gray-900 dark:bg-slate-800 rounded-lg shadow-lg p-6 mb-6">
          <div className="flex gap-4 items-end">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-slate-700 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white"
          ...
          {/* Info Schema */}
          {schemaInfo && (
            <div className="mt-4 p-4 bg-black dark:bg-purple-900/20 rounded-lg">
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {Object.entries(schemaInfo.fields).map(([name, field]: [string, any]) => (
                    <span
                      key={name}
                      className="px-2 py-1 bg-gray-900 dark:bg-slate-700 rounded text-xs"
            ...
            <div className="flex flex-wrap gap-2">
              {(suggestions.length > 0 ? suggestions : exampleQueries).map((q, i) => (
                <button
                  ...
                  className="px-3 py-1 bg-gray-900 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-full text-sm"
                  disabled={!schemaInfo}
                >
                  {q}
                </button>
        ...
        {results && (
          <div className="bg-gray-900 dark:bg-slate-800 rounded-lg shadow-lg p-6">
            ...
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-slate-100 dark:text-white">
            ...
            {/* Errore */}
            {results.error && (
              <div className="mb-4 p-3 bg-black dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b dark:border-slate-700">
                      {Object.keys(results.data[0])
                        ...
                        .slice(0, 6)
                        .map(key => (
                          <th key={key} className="text-left py-2 px-3 font-medium text-slate-300 dark:text-slate-300">
                            {key}
                          </th>
                        ))}
                    </tr>
                  ...
                  <tbody>
                    {results.data.slice(0, 20).map((row: any, i: number) => (
                      <tr key={i} className="border-b dark:border-slate-700 hover:bg-black dark:hover:bg-slate-700/50">
                        {Object.keys(row)
                          .filter(k => !k.startsWith('_'))
                          .slice(0, 6)
                          .map(key => (
                            <td key={key} className="py-2 px-3 text-slate-400 dark:text-slate-400">
        ...
        <div className="mt-8 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg p-6">
          <h3 className="font-semibold text-slate-100 dark:text-white mb-3">
