...
const CatalogCreator: React.FC<CatalogCreatorProps> = ({ customer, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 dark:bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-gray-900 dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-800 dark:border-slate-700 flex justify-between items-center flex-shrink-0">
                    <h2 className="text-lg font-semibold text-gray-200 dark:text-gray-100">{t('catalog_creator.title')}</h2>
                ...
                <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="catalogName" className="block text-sm font-medium text-gray-300 dark:text-gray-300">{t('catalog_creator.catalog_name')}</label>
                            <input type="text" id="catalogName" value={catalogName} onChange={e => setCatalogName(e.target.value)} placeholder={t('catalog_creator.catalog_name_placeholder')} className="mt-1 w-full px-3 py-2 border border-gray-700 dark:border-slate-600 bg-gray-900 dark:bg-slate-700 text-gray-100 dark:text-gray-200 rounded-lg shadow-sm focus:ring-lime-500 focus:border-lime-500 text-sm" />
                        </div>
                        <div>
                            <label htmlFor="supplier" className="block text-sm font-medium text-gray-300 dark:text-gray-300">{t('catalog_creator.select_supplier')}</label>
                            <select id="supplier" value={selectedSupplier} onChange={e => setSelectedSupplier(e.target.value)} className="mt-1 w-full px-3 py-2 border border-gray-700 dark:border-slate-600 bg-gray-900 dark:bg-slate-700 text-gray-100 dark:text-gray-200 rounded-lg shadow-sm focus:ring-lime-500 focus:border-lime-500 text-sm">
                    ...
                    {selectedSupplier && (
                        <div>
                            <h3 className="text-md font-semibold text-gray-200 dark:text-gray-100 mb-2">{t('catalog_creator.products_table_title')}</h3>
                             <div className="relative mb-2">
                                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                    <SearchIcon className="h-4 w-4 text-gray-400" />
                                </div>
                                <input type="text" placeholder={t('catalog_creator.search_products')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-gray-700 dark:border-slate-600 bg-gray-900 dark:bg-slate-700 text-gray-100 dark:text-gray-200 rounded-lg shadow-sm focus:ring-lime-500 focus:border-lime-500 text-sm" />
                            </div>
                            <div className="overflow-auto border border-gray-800 dark:border-slate-700 rounded-lg max-h-[45vh]">
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-black dark:bg-slate-700 sticky top-0">
                                        <tr>
                                            <th className="p-2 w-10 text-center"><input type="checkbox" checked={isAllSelected} onChange={e => handleSelectAll(e.target.checked)} className="h-4 w-4 rounded border-gray-700 dark:bg-slate-900 dark:border-slate-600 text-lime-600 focus:ring-lime-500 cursor-pointer" title={t('catalog_creator.select_all')} /></th>
                                            <th className="p-2 font-semibold text-gray-400 dark:text-gray-300">{t('catalog_creator.product')}</th>
                                            <th className="p-2 font-semibold text-gray-400 dark:text-gray-300 w-28">{t('catalog_creator.price')}</th>
                                            <th className="p-2 font-semibold text-gray-400 dark:text-gray-300 w-28">{t('catalog_creator.unit')}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200 dark:divide-slate-700 bg-gray-900 dark:bg-slate-800">
                                        {filteredProducts.map(product => {
                                            ...
                                            return (
                                                <tr key={product.id} className={info?.selected ? '' : 'bg-black dark:bg-slate-800/50 opacity-60'}>
                                                    <td className="p-2 text-center"><input type="checkbox" checked={info?.selected || false} onChange={e => handleSelectionChange(product.id, e.target.checked)} className="h-4 w-4 rounded border-gray-700 dark:bg-slate-900 dark:border-slate-600 text-lime-600 focus:ring-lime-500 cursor-pointer" /></td>
                                                    <td className="p-2">
                                                        <p className="font-medium text-gray-200 dark:text-gray-200">{product.descrizione.denominazioneLegale}</p>
                                                        <p className="text-xs text-gray-500 dark:text-gray-400 font-mono">{product.identificazione.codiceProdotto}</p>
                                                    </td>
                                                    <td className="p-2"><input type="number" value={info?.price || ''} onChange={e => handleInfoChange(product.id, 'price', e.target.value)} className="w-full text-sm p-1 rounded border-gray-700 dark:bg-slate-700 dark:border-slate-600 focus:ring-lime-500 focus:border-lime-500" /></td>
                                                    <td className="p-2"><input type="text" value={info?.unit || ''} onChange={e => handleInfoChange(product.id, 'unit', e.target.value)} className="w-full text-sm p-1 rounded border-gray-700 dark:bg-slate-700 dark:border-slate-600 focus:ring-lime-500 focus:border-lime-500" /></td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>
                ...
                <footer className="p-4 border-t border-gray-800 dark:border-slate-700 flex justify-end items-center gap-3 flex-shrink-0 bg-gray-900 dark:bg-slate-800">
                    <button onClick={onClose} className="px-4 py-2 border border-gray-700 dark:border-slate-600 text-sm font-medium rounded-lg text-gray-300 dark:text-gray-200 bg-gray-900 dark:bg-slate-700 hover:bg-black dark:hover:bg-slate-600">{t('catalog_creator.close')}</button>
