...
const McpFileSelector: React.FC<McpFileSelectorProps> = ({ files, onSelect }) => {
    return (
        <div className="flex flex-col items-start gap-2 py-2">
            <div className="max-w-2xl w-full px-4 py-3 rounded-xl shadow-sm bg-gray-900 text-gray-200 border border-gray-800">
                {files.length > 0 ? (
                    <>
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto pr-2 -mr-2">
                            {files.map(file => (
                                <button
                                    ...
                                    className="w-full flex items-center gap-3 p-2 rounded-lg text-left hover:bg-gray-900 transition-colors focus:outline-none focus:ring-2 focus:ring-lime-500"
                                >
                                    <FileIcon />
                                    <span className="text-sm font-medium text-gray-300 truncate">{file}</span>
                                </button>
                            ))}
                        </div>
                    </>
