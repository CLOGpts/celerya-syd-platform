...
const DetailSection: React.FC<DetailSectionProps> = ({ title, data, alerts, sectionKey }) => {
    ...
    return (
    <div className="bg-gray-900 dark:bg-slate-800 p-6 rounded-lg border border-gray-800/80 dark:border-slate-700/80 shadow-sm break-inside-avoid">
        <h4 className="text-lg font-semibold text-slate-200 dark:text-slate-200 mb-4 border-b border-gray-800 dark:border-slate-700 pb-3">{title}</h4>
            ...
            {data.map(({ key, label, value }) => {
                ...
                return (
                    <div key={key} className={isIndented ? 'ml-6' : ''}>
                        <dt className={`text-sm flex items-center gap-2 ${isImportant ? 'font-bold text-slate-200 dark:text-slate-200' : 'font-semibold text-gray-400 dark:text-gray-400'}`}>
                            {fieldAlert && (
                        ...
                        <dd className="text-sm text-gray-200 dark:text-gray-300 mt-1 ml-1 whitespace-pre-wrap">{displayValue}</dd>
                    </div>
                );
            })}
        </dl>
