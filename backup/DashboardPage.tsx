...
const AlertsPanel: React.FC<{ alerts: Alert[], status: Status, t: (key: string, params?: any) => string }> = ({ alerts, status, t }) => {
    ...
    return (
        <div className="sticky top-8 bg-gray-900 dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-800 dark:border-slate-700">
            <h3 className="text-xl font-semibold text-gray-200 dark:text-gray-100 mb-2">{t('alerts.panel_title')}</h3>
            <div className="mb-6">
            ...
            {alerts.length === 0 ? (
                <div className="text-center py-8">
                    ...
                    <p className="mt-4 font-semibold text-gray-300 dark:text-gray-200">{t('alerts.no_alerts_title')}</p>
                            ...
                            <ul className="space-y-3">
                                {criticalAlerts.map((alert, index) => (
                                    <li key={`crit-${index}`} className="flex items-start gap-3 text-sm">
                                        ...
                                        <span className="text-gray-300 dark:text-gray-300">{alert.message}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
...
const ContextMenu: React.FC<{
    ...
    return (
        <div ref={menuRef} style={{ top: y, left: x }} className="absolute z-50 bg-gray-900 dark:bg-slate-800 rounded-md shadow-lg border border-gray-800 dark:border-slate-700 py-1 w-48">
            <ul>
                {items.map((item, index) => (
                    <li key={index}>
                        <button
                            onClick={() => { item.onClick(); onClose(); }}
                            className={`w-full text-left px-4 py-2 text-sm text-gray-300 dark:text-gray-300 hover:bg-gray-900 dark:hover:bg-slate-700 hover:text-gray-100 dark:hover:text-gray-100 transition-colors ${item.className || ''}`}
                        >
                            {item.label}
                        </button>
                    </li>
...
const DashboardPage: React.FC = () => {
    ...
    const renderContent = () => {
        if (!selectedCustomer) {
            return (
                 <div>
                    <h1 className="text-3xl font-bold text-gray-200 dark:text-gray-100 mb-2">{t('dashboard.title')}</h1>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                        {customers.map(customer => (
                            <div
                                ...
                                className="group flex flex-col items-center justify-center p-6 bg-gray-900 dark:bg-slate-800 rounded-xl shadow-sm border border-gray-800 dark:border-slate-700 cursor-pointer hover:shadow-lg hover:border-lime-500 dark:hover:border-lime-500 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-lime-500"
                            >
                                ...
                                {renamingCustomerId === customer.id ? (
                                    <input
                                        ...
                                        className="mt-4 text-center font-semibold text-gray-300 dark:text-gray-200 bg-gray-900 dark:bg-slate-700 border border-lime-500 rounded-md w-full"
                                    />
                                ) : (
                                    <span className="mt-4 text-center font-semibold text-gray-300 dark:text-gray-300 group-hover:text-lime-600 dark:group-hover:text-lime-400 transition-colors break-all">{customer.name}</span>
                                )}
                            </div>
                        ))}
                    </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-6 gap-4">
                        <div className="flex items-center gap-4">
                            <button onClick={handleGoToCustomerDashboard} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-400 dark:text-gray-400 transition-colors" aria-label={t('dashboard.back_to_customer_dashboard')}><ArrowLeftIcon className="w-6 h-6" /></button>
                            <div>
                                <h1 className="text-3xl font-bold text-gray-200 dark:text-gray-100">{t('dashboard.results_title')}</h1>
        ...
        return (
             <div>
                <div className="flex items-center gap-4 mb-6">
                    <button onClick={handleGoToCustomerSelection} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-400 dark:text-gray-400 transition-colors" aria-label={t('dashboard.back_to_customer_selection')}><ArrowLeftIcon className="w-6 h-6" /></button>
                    <div>
                        <h1 className="text-3xl font-bold text-gray-200 dark:text-gray-100">{t('dashboard.customer_dashboard_title')}</h1>
                ...
                <div className="mt-12">
                    <h2 className="text-2xl font-bold text-gray-200 dark:text-gray-100 mb-4">{t('dashboard.creation_tools_title')}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <div className="bg-gray-900 dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-800 dark:border-slate-700 h-full flex flex-col items-start text-left">
                            ...
                            <h3 className="text-xl font-semibold text-gray-200 dark:text-gray-100">{t('dashboard.catalog_generator_title')}</h3>
    ...
    return (
        <div className="p-8 bg-black dark:bg-slate-900 min-h-full" onContextMenu={(e) => !contextMenu && handleContextMenu(e)}>
            {renderContent()}
